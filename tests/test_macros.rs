...
fn test_run_single_cmd_with_quote() {
    assert_eq!(
        run_fun!(/bin/echo "hello, rust" | sed r"s/rust/cmd_lib1/g").unwrap(),
        "hello, cmd_lib1"
    );
}
...
fn test_args_with_spaces() {
    ...
    assert!(run_cmd!(mkdir /tmp/$dir; ls -d /tmp/$dir).is_ok());
    assert!(run_cmd!(ls -d /tmp/"$dir").is_ok());
    assert!(run_cmd!(ls -d "/tmp/$dir").is_ok());
    assert!(run_cmd!(rmdir "/tmp/$dir").is_ok());
}
...
fn test_non_eng_args() {
    let msg = "你好！";
    assert!(run_cmd!(/bin/echo "$msg").is_ok());
    assert!(run_cmd!(/bin/echo $msg).is_ok());
    assert!(run_cmd!(/bin/echo ${msg}).is_ok());
}
...
fn test_vars_in_str0() {
    assert_eq!(run_fun!(/bin/echo "$").unwrap(), "$");
}

#[test]
fn test_vars_in_str1() {
    assert_eq!(run_fun!(/bin/echo "$$").unwrap(), "$");
    assert_eq!(run_fun!(/bin/echo "$$a").unwrap(), "$a");
}

#[test]
fn test_vars_in_str2() {
    assert_eq!(run_fun!(/bin/echo "$ hello").unwrap(), "$ hello");
}

#[test]
fn test_vars_in_str3() {
    let msg = "hello";
    assert_eq!(run_fun!(/bin/echo "$msg").unwrap(), "hello");
    assert_eq!(run_fun!(/bin/echo "$ msg").unwrap(), "$ msg");
}

#[test]
/// ```compile_fail
/// run_cmd!(/bin/echo "${msg0}").unwrap();
/// assert_eq!(run_fun!(/bin/echo "${ msg }").unwrap(), "${ msg }");
/// assert_eq!(run_fun!(/bin/echo "${}").unwrap(), "${}");
/// assert_eq!(run_fun!(/bin/echo "${").unwrap(), "${");
/// assert_eq!(run_fun!(/bin/echo "${msg").unwrap(), "${msg");
...
/// assert_eq!(run_fun!(/bin/echo "${}").unwrap(), "${}");
/// assert_eq!(run_fun!(/bin/echo "${").unwrap(), "${");
/// assert_eq!(run_fun!(/bin/echo "${0}").unwrap(), "${0}");
/// assert_eq!(run_fun!(/bin/echo "${ 0 }").unwrap(), "${ 0 }");
/// assert_eq!(run_fun!(/bin/echo "${0msg}").unwrap(), "${0msg}");
/// assert_eq!(run_fun!(/bin/echo "${msg 0}").unwrap(), "${msg 0}");
/// assert_eq!(run_fun!(/bin/echo "${msg 0}").unwrap(), "${msg 0}");
/// ```
...
fn test_pipe_fail() {
    ...
    assert!(run_cmd!(/bin/echo xx | false | wc | wc | wc).is_err());
}
...
/// ```
fn test_redirect() {
    assert!(run_cmd!(/bin/echo xxxx > /tmp/f).is_ok());
    assert!(run_cmd!(/bin/echo yyyy >> /tmp/f).is_ok());
    assert!(run_cmd!(
...
fn test_export_cmd() {
    ...
    assert!(run_cmd!(/bin/echo "from" "builtin").is_ok());
